 ...
 *
 * Ported from the former Step4Options.test.tsx, minus the document-option-toggle tests (those
 * moved to Step4Settings.test.tsx along with the toggle UI itself). Covers: action wiring per use
 * case (download always; claim-only mark-claimed/finish-without-marking; Paperless-gated upload),
 * claim-error banner, the claim-success banner + "view invoices" link, and the hasError/hasBlob
 * action-disabling gate (frontend fix spec item 13 — a failed regeneration invalidates the
 * previous blob, so every action button must disable until a fresh one exists).
 */
...
describe('Step5Actions', () => {
  ...
  it('disables Download/Mark-claimed/Finish while isSaving is true', () => {
    renderStep5({ ...baseProps(), useCase: 'claim', isSaving: true });
  ...
  describe('hasError / hasBlob action gating (a failed regeneration invalidates the blob)', () => {
    ...
    it('a click on a disabled action (hasError true) does not invoke its handler', () => {
      const onDownload = jest.fn();
      renderStep5({ ...baseProps(), useCase: 'claim', hasError: true, onDownload });
