  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        setConfig({ autoItemizeEnabled: false, currency: 'EUR', vatRate: 0.19, llmEnabled: false });
      }
    })();
  }, []);
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (e.key === 'Tab' && showPicker && pickerModalRef.current) {
        ...
        const firstEl = focusableArray[0]!; // guarded by length check at line 149
        const lastEl = focusableArray[focusableArray.length - 1]!; // guarded by length check at line 149
