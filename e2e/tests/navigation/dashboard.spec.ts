 ...
 * 4.  Timeline cards: shows Upcoming Milestones, Work Item Progress, Critical Path
...
// ─────────────────────────────────────────────────────────────────────────────
// Scenario 4: Timeline cards show Upcoming Milestones, Work Item Progress, Critical Path
// ─────────────────────────────────────────────────────────────────────────────

test.describe('Timeline cards (Scenario 4)', { tag: '@responsive' }, () => {
  test('All three timeline cards are visible with work item data', async ({ page }) => {
    const dashboardPage = new DashboardPage(page);
    ...
    try {
      ...
      const upcomingMilestonesCard = dashboardPage.card('Upcoming Milestones');
      await expect(upcomingMilestonesCard.first()).toBeVisible();
      await expect(
        upcomingMilestonesCard.first().getByRole('heading', { name: 'Upcoming Milestones', level: 2 }),
      ).toBeVisible();

      // Work Item Progress card
      const workItemProgressCard = dashboardPage.card('Work Item Progress');
      await expect(workItemProgressCard.first()).toBeVisible();
      await expect(
        workItemProgressCard.first().getByRole('heading', { name: 'Work Item Progress', level: 2 }),
      ).toBeVisible();

      // Critical Path card
      const criticalPathCard = dashboardPage.card('Critical Path');
      await expect(criticalPathCard.first()).toBeVisible();
      await expect(
        criticalPathCard.first().getByRole('heading', { name: 'Critical Path', level: 2 }),
      ).toBeVisible();
    } finally {
      await uninterceptDashboardApis(page);
    }
