...
export const workItemBudgets = sqliteTable(
  'work_item_budgets',
  {
    ...
    includesVat: integer('includes_vat', { mode: 'boolean' }).notNull().default(true),
    createdBy: text('created_by').references(() => users.id, { onDelete: 'set null' }),
    createdAt: text('created_at').notNull(),
    updatedAt: text('updated_at').notNull(),
...
export const subsidyPrograms = sqliteTable('subsidy_programs', {
  ...
  includesNoCategoryItems: integer('includes_no_category_items').notNull().default(0),
  createdBy: text('created_by').references(() => users.id, { onDelete: 'set null' }),
  createdAt: text('created_at').notNull(),
  updatedAt: text('updated_at').notNull(),
});
