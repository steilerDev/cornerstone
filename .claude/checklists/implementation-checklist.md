...
- [ ] **`hidden={…}` is defeated by any author-level `display` on the same element**: the `hidden` attribute hides only via the user-agent rule `[hidden] { display: none }`, and an author stylesheet always beats the UA origin. A CSS-module class declaring `display: flex`/`grid`/`block` on an element that also takes `hidden` leaves it fully laid out while the DOM attribute and `aria-expanded` claim it is collapsed. **Invisible in Jest** — jsdom applies no CSS module rules at all, and jest-dom's `toBeVisible()` is attribute-aware, so both the attribute assertion and the visibility assertion pass on the bug; only a real-browser E2E assertion catches it. Whenever `hidden` toggles a target that also has a layout class, add an explicit `.thatClass[hidden] { display: none; }` guard. Caught in PR for #2046 (`.cardChildren`); the same defect still exists un-fixed at `client/src/components/budget/BudgetLineForm.tsx` (`.currentParentRow`).
- [ ] **A subset array passed to a child that emits indices must be remapped before it indexes the superset**: `DataTable` handed `DataTableColumnSettings` a filtered column list while its `onMoveColumn(from, to)` callback spliced the *unfiltered* `columnOrder`, so drag-reorder moved the wrong column and persisted it to user preferences (#2046). Whenever a component renders `list.filter(...)` and the child reports positions back, translate positions by a stable key, never by offset.
...
- [ ] **A component in `components/` is not automatically shareable**: one born inside a feature keeps that feature's i18n namespace and `data-testid` strings, and mixed generality among its own identifiers is the tell. Parameterize them (`testIdPrefix`, label props, prop-injected strings — prefer prop injection over moving keys to a `common:` namespace) and prove it with a test asserting the *default* testids are absent under a custom prefix. Directory placement alone satisfies the letter of "make this reusable" while leaving the component unusable outside its origin feature (#2060, PR #2063).
...
- [ ] **A generation/epoch guard protects only the value the hook returns, never the side effects the injected callback performs on the way there**: enumerate every `setState` the consumer calls inside the callback and move it to the safe side of the guard (an opaque `meta` passthrough plus post-guard callbacks, not "write the value earlier"). The hook's contract must state that side effects inside the fetch callback are unguarded. Related: diff the hook's `useState` declarations against its reset effect's setters — a reset that clears the data but not the counters describing it is the same class of bug (#2060, PR #2063; applies to the now-shared `useInfiniteScroll`).
...
- [ ] **A new or changed env var must land in every documentation site, not just one**: `server/src/plugins/config.ts` is the source, but the same variable is documented in `.env.example`, the `CLAUDE.md` environment table, `docs/src/getting-started/configuration.md`, and `wiki/Architecture.md`. Diff the read-set (`grep 'getValue('`) against each doc-set before handback, then grep the enablement sentences separately — a stated variable *count* in prose is a second drift surface a name diff cannot see (#1970/PR #1989, #1992, #2023, #2036).
...
- [ ] **Every assertion must be able to fail**: before accepting a test, name the mutation that would break it. Recurring vacuous shapes: `expect(count).toBeGreaterThanOrEqual(1)` where all elements match (use `toBe(N)`, and revert each call site individually), key-echo fixtures that assert their own input back, and an "unmocked render" whose expectations read the *input* content tree or a quantity fixed by construction (a table width that is `printableWidth()` for any input). A render happening is not a measurement (#1910/PR #2004, #1953/PR #2035, #2001/PR #2007).
- [ ] **`DataTable` mounts the desktop and mobile DOM simultaneously**, so every `data-testid` it renders without a per-viewport `renderCard` override exists twice. `getAllByTestId(x)[0]` is blind to this and silently asserts against whichever copy sorts first. Scope the locator to the viewport container, or give each rendering a distinct testid. Note that `beta`'s `Quality Gates` never runs the E2E suite that would surface it (#2046, PR #2068; systemic sweep deferred to #2069).
...
- [ ] **An AC whose premise contradicts the code is a defect in the acceptance record, not a design constraint**: when an AC names a symbol, default, or behaviour that does not exist, stop and route it back for amendment — never silently design around it, and never implement the misdescription (a correct implementation then fails at UAT). Seen 4×.
- [ ] **Premises go stale at *pickup*, not at filing**: an issue filed by another agent is a snapshot of that round's codebase. When several queue items touch one area, an earlier merge invalidates a premise that was accurate when written (#1941 and #1950 were both correct when filed and invalidated 1–2 days later). Re-verify every premise against `HEAD` when the item is picked up.
