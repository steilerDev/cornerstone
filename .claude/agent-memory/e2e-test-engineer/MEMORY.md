...
- **Shard 6 root cause (PR #1676)**: Scenario 6 (photo attach happy path) changed from `photo-upload-zone` to `'Upload Photos'` button assertion for desktop. After `upload1Promise` resolves, uploads may still be in-flight and the button is `disabled=true` with text "Uploading..." — timing-dependent. Fixed in commit c0a3d934 with viewport-conditional logic.
