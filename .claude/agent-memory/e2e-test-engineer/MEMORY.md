...
- **`**/api/photos` glob matches query strings**: Playwright's `**` translates to `(.*)` regex which matches the full URL including `?query=string`. Use distinct routing: POST-only handler checks `route.request().method()`, GET with query params use URL predicate function route.
...
- **diary-drafts photo-upload-zone NOT on touch devices**: `data-testid="photo-upload-zone"` only exists on non-touch (desktop) layout. On touch devices (tablet/mobile, `hover:none`), `mobileButtonPair` div renders instead. Use `getByRole('button', { name: 'Upload Photos' }).first()` as a viewport-agnostic alternative — aria-label is static even when button shows "Uploading...". For Scenario 17 (responsive), use viewport-conditional logic: `viewportWidth <= 1024` = touch device.
