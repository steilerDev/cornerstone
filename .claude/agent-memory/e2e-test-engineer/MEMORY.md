...
- `autoItemizeError` banner renders inside the `<section>` (NOT a portal) as `<div role="alert">`. Located via `budgetLinesSection.locator('[role="alert"]')`.
- Mismatch warning: `[class*="warningBlock"]` inside preview modal. Currency amounts rendered by `formatCurrency` — German locale uses period thousands separator: `1.700`, `2.000`.
