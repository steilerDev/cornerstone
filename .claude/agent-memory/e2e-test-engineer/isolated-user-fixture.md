...
description: e2e/fixtures/isolatedUser.ts — how to give a preference-mutating spec its own user; plus the hard server-side limits (login rate limit, soft user delete, 100-row user table) that dictate per-worker vs per-test scope
metadata:
  type: project
---
...
- `DELETE /api/users/:id` is a **soft delete** (`deactivateUser` + `destroyUserSessions`) —
  ...
  accumulate.
- `/settings/users` renders **100 rows per page** with `sortBy: null` (insertion order), and
  `edit-user.spec.ts` / `deactivate-user.spec.ts` find their user by scanning the rendered
  page. Piling up deactivated E2E users past 100 breaks those unrelated specs. Converting all
  ~34 dashboard tests per-test would have added ~85 users/run; per-worker adds ~15.
