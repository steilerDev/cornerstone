...
- [PR #2070 — SearchPicker dropdown-open test timeout bisection](pr2070-searchpicker-dropdown-timeout.md) (2026-09-07) — CI timeouts across ~16 SearchPicker-family test files after a jest/testing-library bump; scratch-`npm install`-downgrade bisection proved NOT a dependency regression; **`projects[].testTimeout` is a silent no-op in jest-circus — must be set at jest.config.ts top level**; plus a genuine debounce-vs-query-agnostic-mock race in HouseholdItemPicker.test.tsx fixed by asserting the mock call inside `waitFor`, not a timeout bump.
