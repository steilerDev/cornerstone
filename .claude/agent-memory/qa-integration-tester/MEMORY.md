...
- [PR #2070 — SearchPicker dropdown-open test timeout bisection](pr2070-searchpicker-dropdown-timeout.md) (2026-09-07) — CI timeouts in WorkItemPicker/InvoicePaperlessPickerModal tests after a further jest/testing-library bump; scratch-`npm install`-downgrade bisection proved NOT a dependency regression (identical failure on old versions); root cause is legitimate first-time FloatingPortal-mount slowness under CPU contention, not a hang; fixed via file-level `jest.setTimeout(60000)`.
