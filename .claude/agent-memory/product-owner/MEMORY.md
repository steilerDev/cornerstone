...
10 stories created (#387-#394, #413, #415). See [epic-04-planning.md](epic-04-planning.md) for full details.
Story 4.10 (#415): Household Item Timeline Dependencies & Delivery Date Scheduling. Replaces simple `household_item_work_items` M:N association with `household_item_deps` dependency table supporting work item AND milestone predecessors, all 4 dependency types, lead/lag days. Scheduling engine extended to compute earliest/latest delivery dates. Migration drops old junction table, preserves data as FS deps. Gantt chart and Calendar show HIs with visual distinction. 11 ACs, 14 UAT scenarios. Status: Todo.
