...
- **2026-05-17** — 1 standalone diary UX bug #1446 (follow-up to #1435 / PR #1436): replace "Hide drafts" checkbox in DiaryFilterBar with a 4th "Drafts" chip inline with mode chips (All/Manual/Automatic). Independent toggle (NOT part of exclusive mode group), default active = drafts visible, muted color to signal different filter axis. 7 ACs. Frontend-only — i18n rename `filterBar.hideDrafts` → new key (e.g. `filterBar.draftsChip`), test-id rename `hide-drafts-checkbox` → `status-filter-drafts`. Test callers to update: `DiaryFilterBar.test.tsx` (Scenarios 12-16), `DiaryPage.test.tsx` (Scenarios 9-11), `e2e/tests/diary/diary-drafts.spec.ts`, `e2e/pages/DiaryPage.ts`. Todo.
