...
- **#1677** — [auto-itemize] Line total ignores VAT for net (`includesVat=false`) lines. `computedLineTotal` (`AutoItemizePage.tsx:656`), `computeWarnings` extractedTotal (`invoiceAutoItemizeService.ts:85`), and `totalItemized` validation (`invoiceAutoItemizeService.ts:457/462`) all sum raw `totalAmount` ignoring VAT, so net lines undercount ~19% → false variance/mismatch, false `TOTAL_MISMATCH`, undercounted `ItemizedSumExceedsInvoiceError`. Fix: contribute gross (net × 1.19) per `effectivePlannedAmount()` in `shared/src/types/budget.ts`. Auto-itemize mini-epic (#1545-#1547) closed without a parent epic, so this is a standalone bug. (Todo, 2026-06-15). Related to VAT/quotation cluster #1439-#1441.
