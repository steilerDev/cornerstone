...
### Issues filed off PR #1959 (2026-08-03)

**#1965** legend (Must Have — reviewed below), **#1966** column-toggle E2E, **#1967** `attachmentsNote` override unreachable, **#1968** meta-suffix single run, **#1969** `testPrefix`/`authenticatedPage`, **#1970** configurable auth rate limits, **#1971** `search-users.spec.ts` leftovers, **#1972** silent failed column-pref saves + dead `isLoaded`.
...
## #1965 review — PR #1979 CHANGES_REQUIRED (2026-08-03)

Production change was correct and minimal (16 lines in `buildReportContent.ts`): legend gates on `splitInvoiceIds.size`/`depositReducedInvoiceIds.size`, which are populated **only from included invoices** and are the exact same sets the row flags read, so legend↔row correspondence is exact by construction. Both `overviewPdf.ts:857` and `ReportContentEditor.tsx:458-464` already rendered `${marker}: ${text}`, and `applyOverrides`/`applyAiContent` copy `footnotes` through untouched — so the legend is genuinely non-editable, which was the story's own requirement. 8 of 12 ACs met.

**Rejected on two HIGH findings:**

1. **Preview renders `partial:Amount shown…` with no space.** `ReportContentEditor.tsx:461-462` puts `{note.text}` on the line after `</span>`; JSX strips newline-containing whitespace, and `.footnoteMarker` has no trailing margin. PDF has the space. **Landed on this PR, not filed as pre-existing**, because #1959 had made the code path unreachable (`footnotes` always empty) — this PR is what makes it reachable. Same precedent as #1945/M1.

2. **AC 1.5 was asserted by exactly the method it forbids.** It says "asserted by counting occurrences in the rendered preview DOM and in the rendered PDF text, **not only by inspecting the array length**"; the PR asserts `toHaveLength(1)`. AC 1.4 likewise array-only; AC 3.1's preview surface has zero coverage; the **split sentence is never asserted present in any rendered output** (only the deposit one is, at `realRender.test.ts:1055`).

**The generalizable ruling — why a coverage gap was blocking here.** #1965 exists because #1959 emptied `footnotes[]` and the only guard was `expect(content.footnotes).toEqual([])`, an array-level assertion that was simply _updated_ to match the removal. A fix that re-lands the behaviour behind the same class of assertion leaves the identical regression channel open. **What settled it: finding 1 is the proof.** The rendered-DOM assertion the AC asked for and the PR skipped would have failed on this very commit. A coverage gap stops being bureaucratic the moment you can name the live defect it concealed — look for that before rejecting on coverage alone, and lead with it when you do.

Contrast the waiver cases: #1931 ACs 3.2/3.3 (unverifiable → merge, UAT gates Done) and #1909 AC 4.6 (unverifiable + substitute assertion → documented deviation). **Verifiable-but-unimplemented is neither** — it is a gap, and on a Must Have it blocks.

Non-blocking, recorded on the PR: AC 4.1 has no measured layout assertion (real risk low — longest unbreakable token `Abschlagszahlungen.` ~19 chars; machinery exists at `realRender.test.ts:657` `PRINTABLE_WIDTH_PT = 515.28` and the y-position pattern at :2000-2030); `ReportContentEditor.test.tsx:938-955` still feeds `†`/`‡` as fixture markers, which is _why_ finding 1 was invisible; `depositReduced` dedup untested; `buildReportContent.ts:236,244` re-calls the label key instead of reusing `labels.splitNote` (built later at L293, so reuse needs a reorder).

**Verdict posted via `gh pr comment` with an explicit `## Verdict:` line** — `gh pr review` cannot be used, the PR author is the token owner (steilerDev). Same constraint as #1909. (Round 2 confirmed the exact error: `gh pr review --approve` → `GraphQL: Review Can not approve your own pull request (addPullRequestReview)`. Try it first, fall back to `gh pr comment`.)

### Round 2 — APPROVED (2026-08-03, head `d360e1d8`)

Both HIGH findings fixed. Separator fix is `<span …>{note.marker}:</span> {note.text}` **on one line** (`ReportContentEditor.tsx:461`) — JSX only strips whitespace that spans a newline, so a same-line space is preserved; `{' '}` was not needed. Finding 2 closed on a *better* surface than I specified: the rendered-preview occurrence count arrived as **product-architect's HIGH-1 fix** (Scenario 18 had to be inverted from `toHaveCount(0)` to `1` with two split invoices), which is exactly AC 1.5's demand in a real browser. Verified *executed*, not just written — E2E shard 2/16 test 161. Lesson: **when another reviewer's blocking finding forces a test change, check whether it also discharges your own coverage finding before re-demanding it.**

Two spontaneous strengthenings worth reusing as asks: `expect(footnotes[0].marker).toBe(content.labels.splitNote)` turns AC 2.3's shared-token property from *inferred* (two call sites use the same key) into *asserted*; and deleting the page-object comment "Never assert a positive count on these" — a locator documented as negative-only is one nobody will use to catch the regression again.

**Ruling on the residuals.** AC 4.1 (measured layout) was still not met, and 3 assertion gaps remained (AC 1.5 PDF-text count, AC 1.4 PDF-text negative, `depositReduced` dedup + no rendered-preview coverage for the deposit sentence at all). Approved anyway because each is covered **transitively**: array length is pinned *and* `overviewPdf.ts`'s verbatim append is pinned by its own case, so neither a duplicate nor a deletion can reach the PDF without failing something. **A sound composition of separately-pinned legs is real coverage — don't reject for the literal AC surface when the property is closed.** But an unmet AC must not be absorbed by "approved with follow-ups": filed **#1980** (tech-debt, Should Have, Backlog) with AC 4.1 as criterion 1.

Two MUST FIX (approve + fix before merge, both one-liners, neither an AC gap): **M1** the just-fixed separator is unguarded — the unit test matches marker and text *independently* and the E2E matches the sentence in concatenated `main.textContent()`, where `partial:Amount` and `partial: Amount` are indistinguishable; fix with `toHaveTextContent('partial: <sentence>')` on the `<li>` (it collapses whitespace runs but never *inserts* any, so it does catch a missing space — and the collapse turns the marker's U+00A0 into a plain space, retiring the `/^less\sdeposit:$/` workaround). **M2** the E2E section banner (`:1643-1645`) and the `test()` **title** (`:1649-1650`) still say "no footnote list anywhere on the page" while the body asserts count 1 — the title is the worse one because it is what CI output and the Playwright report advertise, and it invites a future author to "fix" the assertions back to 0. **Generalizable: a stale test *name* is worse than a stale comment — it is the regression's own invitation.**
