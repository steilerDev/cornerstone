...
## Coverage gaps: when an AC-mandated assertion method is itself blocking (PR #1979, #1965)

- **Blocking a PR on test-coverage alone needs a named live defect, and usually there is one.** #1979's production change was correct; I rejected it because AC 1.5 said "not only by inspecting the array length" and the PR asserted exactly that. What made the rejection more than checklist enforcement: the rendered-DOM assertion the AC asked for **would have failed on that commit** — the preview rendered `partial:Amount shown…` with no space after the marker. Hunt for the concealed defect before rejecting on coverage, and lead the review with it.
- **Where regression-fix stories hide their gap.** A story that exists because output silently disappeared must be guarded at the **rendered surface**, not at the intermediate data structure. #1965 exists because #1959 emptied `footnotes[]` and the only guard was `expect(content.footnotes).toEqual([])` — an array-level assertion that a removal PR simply _updates_ to match. Check that a fix does not re-land behind the same class of assertion that failed to catch the original.
- **Both surfaces means both surfaces.** With one content model feeding a preview and a PDF, "renders identically" ACs get satisfied on the cheaper surface. #1979 asserted the PDF tree for one of two sentences and the preview for neither. Also check per-item: N sentences × M surfaces × 2 locales, not "the feature is asserted somewhere".
- **JSX whitespace is a real display-defect class.** `<span>{marker}:</span>` on one line and `{text}` on the next produces no space — Babel strips newline-containing whitespace between children. Check for a companion `margin-inline-end` on the label class before assuming a gap renders. The PDF sibling (`` `${marker}: ${text}` ``) having the space is what makes it a divergence.
- **Stale test fixtures mark the blind spot.** `ReportContentEditor.test.tsx` still fed `†`/`‡` as footnote markers months after markers became words. A fixture whose values can no longer occur in production means that render path is effectively untested — treat it as a pointer to look, not just a tidy-up.
- **Unreachable-then-reachable = this PR's defect.** A latent bug in a code path that the PR under review is what re-enables belongs in that PR, not in a follow-up. Same call as #1945/M1.
- **Taxonomy for unverified ACs:** unverifiable + substitute assertion → documented deviation (#1909 AC 4.6); unverifiable, no substitute → merge the code, gate Done on UAT (#1931 ACs 3.2/3.3); **verifiable but unimplemented → gap, blocking on a Must Have** (#1979).

### Round 2 of the same PR — how the coverage finding gets discharged

- **A composition of separately-pinned legs is real coverage.** #1979 r2 still asserted the dedup count only on the array, never on the rendered PDF text — the literal AC surface. Approved anyway: array length is pinned _and_ `overviewPdf.ts`'s verbatim append is pinned by its own case, so neither a duplicate nor a deletion reaches the PDF without failing something. **Ask whether the property is closed, not whether the named surface is covered.** This is the counterweight to the r1 rule above — without it, "coverage gap" enforcement becomes exactly the bureaucracy r1 disclaimed.
- **Check whether another reviewer's blocking finding already discharged yours.** My AC 1.5 rendered-preview occurrence count arrived as the fix for the architect's unrelated HIGH (an E2E scenario asserting the opposite of the new behaviour forced Scenario 18 to flip from `toHaveCount(0)` to `1`, with two split rows in the fixture). Re-demanding it would have been noise. Read the other reviewers' findings and the commits that answered them before writing round 2.
- **Verify a test was _executed_, not just written.** Grep the shard logs for the test title (`gh api repos/<r>/actions/jobs/<id>/logs`) — a green suite plus a new assertion is not proof the assertion ran in the shard you think. Sharding put the spec's desktop project in shard 2 while its sibling scenario ran in shard 13.
- **An unguarded correct fix is MEDIUM, not HIGH.** Once the defect is fixed on disk, the missing regression guard has no live defect behind it — so by the r1 rule it stops being blocking. Approve with a MUST FIX and the exact assertion.
- **`toHaveTextContent` guards a missing separator; two independent `getByText` calls do not.** jest-dom collapses runs of whitespace but never _inserts_ any, so `toHaveTextContent('partial: <sentence>')` fails on `partial:<sentence>`. The collapse also turns U+00A0 into a plain space — type the expectation with a plain space and the `/^less\sdeposit:$/` regex workaround becomes unnecessary. Beware the E2E equivalent: `main.textContent()` concatenation makes `partial:Amount` and `partial: Amount` indistinguishable.
- **A stale test _name_ is worse than a stale comment.** #1979 updated the spec docstring and in-body comments but left the section banner and the `test()` title saying "no footnote list anywhere on the page" for a test that now asserts one exists. The title is what CI output and the Playwright report advertise, so it actively invites a future author to "fix" the assertions back — re-breaking the very thing the story repaired. Check names and banners, not just comment bodies, whenever a PR inverts an assertion.
- **Don't let "approved with follow-ups" absorb a flatly unmet AC.** #1979's AC 4.1 (measured layout) went unaddressed across both rounds; it became criterion 1 of a filed, boarded issue (#1980) rather than a bullet in a review comment nobody re-reads.
