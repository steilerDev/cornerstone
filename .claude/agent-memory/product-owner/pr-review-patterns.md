...
- **A constraint asserted only in the prompt is not a guarantee.** Pattern to check on any LLM story: for each behavioural rule the prompt states, ask what fails if the model ignores it. `prompts.test.ts`-style tests pin that the *instruction exists*, which is real coverage of the instruction and zero coverage of the outcome. #1931 got this right by construction (stated caps derived from `REPORT_CONTENT_LIMITS`); #1932's AC 1.6 plain-prose rule had no enforced side at all, and the render path (`applyAiContent.ts` → pdfmake `{ text }`) is literal, so `**bold**` reaches a bank-facing PDF.
- **Coerce vs reject at an LLM boundary — match the policy the field already has, and weigh blast radius per call.** One generation often produces several unrelated outputs (subject + body + all per-invoice descriptions). Rejecting the whole response over a cosmetic defect in one field discards correct expensive output and may fail identically on retry. If the validator already *truncates* rather than throws on that field, a stricter failure mode for a *milder* violation is incoherent. Ruled *strip, not reject* on #1952.
...
- **Amend an honest interim doc statement, don't delete it.** When a PR documents "instructed but not enforced" and a follow-up adds enforcement, the follow-up AC must say *amend that bullet* — otherwise the page keeps a stale "not enforced" line.
