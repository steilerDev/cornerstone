...
- **`timeWindow: 0` disables `@fastify/rate-limit` entirely** (v11.1.0). `LocalStore.incr` tests `iterationStartMs + timeWindow <= nowInMs` → always true at 0 → counter resets to 1 on every request, never reaches `max`. Demonstrated by requiring `node_modules/@fastify/rate-limit/store/LocalStore.js` directly and looping `incr` — no app build needed, deterministic, and far better review evidence than an argument. Reuse that technique for any store/limiter claim.
