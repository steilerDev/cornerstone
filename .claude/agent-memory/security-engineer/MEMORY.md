...
Most recent: **#2015** feat(reports): `SourceReportInvoice.splitKind` ('lines'|'deposits'|'both'|null), SQL UNION rewrite in `sourceReportService.ts` step f (#1911) — APPROVED, no findings. Requested despite missing the literal auth/Dockerfile/dependency triggers because it rewrites a SQL query and adds a field carrying info about *other* budget sources. Verified (not trusted): `sourceId`/`joinedInvoiceIds` genuinely parameter-bound via drizzle `sql\`\`` template + `sql.join(ids.map(id => sql\`${id}\`), sql\`, \`)`; the `'line'`/`'deposit'` origin tags are hardcoded SQL literals, no variable reaches them. Independently re-verified the "no auth boundary between budget sources" premise myself rather than taking product-owner's word: `budgetSources` schema (schema.ts:333) has zero owner/household column, every route in `budgetSources.ts` gates only on `if (!request.user)` — no per-source ACL exists anywhere in the app, so `splitKind` can't leak across a boundary that doesn't exist. Confirmed `splitKind` only ever drives two boolean comparisons client-side (`buildReportContent.ts`) that toggle pre-existing fixed inline labels — the categorical string itself never reaches rendered output, logs, or any other response field. No DoS: new `MAX(CASE...)` aggregates computed in the same GROUP BY pass, pre-existing indexes on `invoice_id` untouched. **Reusable premise-check pattern**: when a reviewer/agent asserts "no auth boundary exists here" as the basis for an info-disclosure verdict, independently confirm via schema (no owner/household FK) + routes (no requireRole/ACL filter) before accepting — don't take it on trust even from a normally-reliable source.

Previous: **#1999** fix(security): null-safe `normalizeIP` keyGenerator (CVE-2026-15144 follow-up) — COMMENTED APPROVED. Adds `keyGenerator: (request) => normalizeIP(request.ip ?? 'unknown')` to rateLimitPlugin.ts. CVE remains closed: normalizeIP is a named export in @fastify/rate-limit 11.2.0 (`export function normalizeIP(ip: string, ipv6Subnet?: number): string`; defaults to /64); `'unknown'` fallback returns `'unknown'`.toLowerCase() (not IPv6 → first branch of function) so null-IP requests share one conservative bucket rather than throwing. 1 low (null path `?? 'unknown'` has no unit test — can't trigger via app.inject which defaults remoteAddress to 127.0.0.1); 1 informational (ipv6Subnet config drift: custom generator hardcodes /64 default, ignores any future `ipv6Subnet` plugin option). Updated Security-Audit.md: fixed stale L91 snippet (`req.ip` → null-safe normalizeIP), updated CVE finding remediation to document two-PR fix + key-must-be-normalizeIP invariant.

Previous: **#1998** fix(security): #1995 — CVE-2026-15144 / GHSA-grpc-p53c-r64v (CVSS 7.3) IPv6 /64 rate-limit bypass. Custom `keyGenerator` in `rateLimitPlugin.ts` returned raw `request.ip` string (full IPv6 address) rather than /64-normalized prefix; an attacker with a /64 allocation could exhaust 2^64 distinct buckets and bypass rate limits on all 4 auth endpoints entirely. Fix: delete the 13-line custom `keyGenerator` — `@fastify/rate-limit` 11.2.0's default generator applies /64 normalization. COMMENTED APPROVED. 1 informational: only login route covered by the new /64-sharing test; setup/password/dav-tokens endpoints share the same plugin instance so normalization is guaranteed, but explicit tests are missing.
