...
- **household_items.url field (PR #396)**: Stores user-provided retailer URLs — stored as text only, never fetched server-side in this PR. Future API stories must ensure no server-side fetch (SSRF risk) and frontend renders with rel="noopener noreferrer".
