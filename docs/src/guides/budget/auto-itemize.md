...
When you choose **Create Budget Line**, the picker closes immediately and the new line is *queued* rather than created on the spot: the card shows a **"Creating New"** badge with an inline form for the new line's details and a **Discard** button to back out. The actual budget line is only written when you **Save** the whole page -- so you can keep reviewing the other rows without interruption, and nothing is committed until you are ready. While a queued line is active, that card's category and funding-source pickers step aside (the new line carries its own).

The card's **"Price includes VAT"** checkbox stays in sync with the queued line in both directions. Toggling it on the line card updates the queued draft, and editing it inside the draft form updates the card -- so whatever you set is what gets saved. This matters most for **VAT-exclusive (net) invoice lines** assigned to a work item: unchecking **"Price includes VAT"** after queuing the new line now correctly records the line as net and computes the gross amount Cornerstone stores from it. (Previously the change could be silently dropped on save.)
