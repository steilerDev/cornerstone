...
try {
  ...
  if (/\bgit\s+push\b/.test(cmd)) {
    if (/\bpush\b[^;|&]*\s(?:origin|upstream)\s+(?:[^\s:]+:)?(?:main|beta)(?![\w./-])/.test(cmd)) {
      block(
  ...
  if (/\bgit\s+commit\b/.test(cmd)) {
    ...
    // Human escape hatch (mirrors check-trailers.sh): a commit with no Claude
    // trailer at all is treated as human-authored and skipped.
    if (!/claude\s+[a-z0-9-]+\s*[<(]/i.test(cmd)) process.exit(0);
